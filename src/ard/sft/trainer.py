...
from ard.sft.data import SFTSample, load_mixed_sft_samples
from ard.sft.loss import anchor_kl_loss, weighted_ce_loss
from ard.sft.schema import ARDSFTConfig
from ard.modeling import build_peft_config, ensure_tokenizer_ready, save_lora_adapter


class SFTSampleDataset(Dataset[SFTSample]):
    def __init__(self, samples: list[SFTSample]) -> None:
...
class ARDSFTTrainer:
    ...
    def train(self) -> None:
        ...
        dataset = SFTSampleDataset(samples)
        ...
        if teacher is not None:
            model, teacher, optimizer, dataloader = accelerator.prepare(
                model, teacher, optimizer, dataloader
            )
        else:
        ...
        for epoch in range(self.config.training.total_epochs):
            for batch in dataloader:
                ...
                if (
                    ...
                    and global_step % self.config.training.logging_steps == 0
                ):
                    print(
                        json.dumps(
                            {
